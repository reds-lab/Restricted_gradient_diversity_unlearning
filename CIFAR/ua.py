...
def calculate_ua_stats(
    ...
    return 1 - (classes == expected_class).mean()
...
@click.option('--class',  'expected_class',             help='The unlearned (and generated) class', metavar='INT', type=int, default=0, show_default=True)
...
@click.option('--out_path', 'ua_out_path', help='Path to write UA', metavar='PATH|ZIP',              type=str, default='./results/ra.txt', required=False)

def main(image_path, model_path, expected_class, seed, batch, ua_out_path=None):
    """Calculate UA for a given set of images."""

    ua = calculate_ua_stats(
        image_path=image_path, model_path=model_path, expected_class=expected_class,
        seed=seed, max_batch_size=batch)

    print("UA:", ua)

    if ua_out_path is not None:
        os.makedirs(os.path.dirname(ua_out_path), exist_ok=True)        
        with open(ua_out_path, 'a') as f:
            f.write(f'{ua:g}\n')
