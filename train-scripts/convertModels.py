...
def savemodelDiffusers_rgd(name, proposed_path, forget, retain, compvis_config_file, diffusers_config_file, device="cpu"):
    checkpoint_path = f"models/{proposed_path}/{forget}_{retain}/{name}/{name}.pt"
    ...
    prediction_type = "epsilon"
    extract_ema = False
    dump_path = f"models/{proposed_path}/{forget}_{retain}/{name}/{name.replace('compvis','diffusers')}.pt"
    upcast_attention = False
